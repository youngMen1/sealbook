...
进行本项重构之前，你应该先寻找其他替换方案。你应该观察魔法数如何被使用，而后往往你会发现一种更好的使用方式。如果这个魔法数是个type code（型别码）， 请考虑使用[以类取代型别码](http://wangvsa.github.io/refactoring-cheat-sheet/organizing-data/#_13)；如果这个魔法数代表一个数组的长度，请在遍历该数组的时候，改用Array.length\(\)。
...
[封装值域](http://wangvsa.github.io/refactoring-cheat-sheet/organizing-data/#_7)是封装过程的第一步。通过这项重构手法，你可以将数据隐藏起来，并提供相应的访问函数（accessors）。但它毕竟只是第一步。如果一个class除了访问函数（accessors）外不能提供其他行为，它终究只是一个dumb class \(哑类〕。这样的class并不能获得对象技术的优势，而你知道，浪费任何一个对象都是很不好的。实施[封装值域](http://wangvsa.github.io/refactoring-cheat-sheet/organizing-data/#_7)之后，我会尝试寻找那些使用「新建 访问函数」的函数，看看是否可以通过简单的[搬移函数](http://wangvsa.github.io/refactoring-cheat-sheet/moving-features-between-objects/#_3)轻快地将它们移到新对象去。
