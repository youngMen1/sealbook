...
关于日期时间的计算，我先和你说一个常踩的坑。有些同学喜欢直接使用时间戳进行时间计算，比如希望得到当前时间之后 30 天的时间，会这么写代码：直接把 new Date\(\).getTime 方法得到的时间戳加 30 天对应的毫秒数，也就是 30 天 \_1000 毫秒 \_3600 秒 \*24 小时：
...
**回答：**保存和传输用时间戳的确比较方便，用带有时区的字面量时间表示字符串，还有格式不统一的问题
